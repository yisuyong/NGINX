...
typedef struct ngx_http_log_div_var_s ngx_http_log_div_var_t;
...
struct ngx_http_log_div_var_s{
    ngx_uint_t	count;
    ngx_uint_t remaining_msec;
    ngx_uint_t div_msec;
    ngx_uint_t current_count;
    off_t sent_bytes_per_count;
    off_t sent_bytes_per_count_header;
    off_t sent_bytes_remaining;
    off_t sent_bytes_remaining_header;
...
static u_char *ngx_http_log_substatus(ngx_http_request_t *r, u_char *buf,
    ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t);
...
static u_char *ngx_http_log_body_bytes_sent(ngx_http_request_t *r,
    u_char *buf, ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t);
...
static ngx_http_log_var_t  ngx_http_log_vars[] = {
    ...
    { ngx_string("substatus"), NGX_INT_T_LEN, ngx_http_log_substatus },
...
//#jimmy-4-6 start
    ...
    div_var_t.count=1;
    div_var_t.remaining_msec=0;
    div_var_t.current_count=1;
    div_var_t.sent_bytes_per_count = 0;
...
//#jimmy-4-7 start
    ...
    {
        ngx_uint_t	Bps=0;
        ngx_uint_t	Bps_header=0;
        ngx_uint_t	request_time_msec=0;
        u_char request_time_str[NGX_TIME_T_LEN + 4];

	ngx_memzero(request_time_str, sizeof(request_time_str));
	ngx_http_log_request_time(r,request_time_str,NULL,&div_var_t);

	request_time_msec=ngx_atofp(request_time_str,ngx_strlen(request_time_str),3);

	if(div_var_t.div_msec < request_time_msec)	
	{	
		div_var_t.count=(((ngx_uint_t)request_time_msec) / (div->sectime * 1000))+1;
		div_var_t.remaining_msec=(((ngx_uint_t)request_time_msec) % (div->sectime * 1000));

		Bps=(r->connection->sent - r->header_size)/ (request_time_msec / 1000);
		Bps_header=(r->connection->sent - r->header_size)/(request_time_msec/1000);

  	        div_var_t.sent_bytes_per_count = Bps * div->sectime;
        	div_var_t.sent_bytes_per_count_header = Bps_header * div->sectime; 

		div_var_t.sent_bytes_remaining= (r->connection->sent - r->header_size) - (div_var_t.sent_bytes_per_count * (div_var_t.count - 1) );
...
//    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"suyong http log : %s",buf);
    if (log->script == NULL) {
...
//    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"suyong http test9-1 log : %s",buf);  //로그 값 사이 스페이스,\" 추가
    return buf;
}
...
ngx_http_log_copy_long(ngx_http_request_t *r, u_char *buf,
    ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t)
{
//    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"suyong http test9-2 log : %s",(u_char *) op->data); 
    return ngx_cpymem(buf, (u_char *) op->data, op->len);
}


static u_char *
ngx_http_log_pipe(ngx_http_request_t *r, u_char *buf, ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t)
{
    if (r->pipeline) {
        *buf = 'p';
    } else {
...
static ngx_tm_t ngx_http_log_div_time_renew(ngx_http_log_div_var_t *div_var_t)
{
        ...
        div_tp.sec=tp->sec;
        div_tp.msec=tp->msec;
        div_tp.gmtoff=tp->gmtoff;

        temp=tp->msec - (div_var_t->remaining_msec%1000);
        if(temp<0)
        {
                div_tp.sec=div_tp.sec - 1;
                div_tp.msec = div_tp.msec +1000;
        }
        div_tp.msec=div_tp.msec - (div_var_t->remaining_msec%1000);
        div_tp.sec=div_tp.sec - (time_t)(div_var_t->remaining_msec/1000) - (time_t)((div_var_t->div_msec/1000) * (div_var_t->count - div_var_t->current_count - 1 ));


#if (NGX_HAVE_GETTIMEZONE)

    div_tp.gmtoff = ngx_gettimezone();
    ngx_gmtime(div_tp.sec + div_tp.gmtoff * 60, &tm);

#elif (NGX_HAVE_GMTOFF)

    ngx_localtime(div_tp.sec, &tm);
    div_tp.gmtoff = (ngx_int_t) (tm.ngx_tm_gmtoff / 60);

#else

    ngx_localtime(div_sec, &tm);
    div_tp.gmtoff = ngx_timezone(tm.ngx_tm_isdst);
...
static u_char *
ngx_http_log_time(ngx_http_request_t *r, u_char *buf, ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t)
{
    ...
    if(div_var_t->count != div_var_t->current_count)
    {
        ngx_tm_t         tm;
        ngx_time_t  *tp;
        tp = ngx_timeofday();

	tm=ngx_http_log_div_time_renew(div_var_t);

         return ngx_sprintf(buf, "%02d/%s/%d:%02d:%02d:%02d %c%02i%02i",
...
ngx_http_log_time_korean(ngx_http_request_t *r, u_char *buf, ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t)
{
    if(div_var_t->count == 1)
    {
    	//return ngx_cpymem(buf, ngx_cached_err_log_time.data,ngx_cached_err_log_time.len);
    	return ngx_cpymem(buf, ngx_cached_http_log_korean.data,ngx_cached_http_log_korean.len);
    }
    if(div_var_t->count != div_var_t->current_count)
    {
        ngx_tm_t         tm;
...
static u_char *
ngx_http_log_iso8601(ngx_http_request_t *r, u_char *buf, ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t)
{
    if(div_var_t->count == 1)
    {
	...
	tm=ngx_http_log_div_time_renew(div_var_t);
        ...
        return  ngx_sprintf(buf, "%4d-%02d-%02dT%02d:%02d:%02d%c%02i:%02i",
                       tm.ngx_tm_year, tm.ngx_tm_mon,
                       tm.ngx_tm_mday, tm.ngx_tm_hour,
                       tm.ngx_tm_min, tm.ngx_tm_sec,
                       tp->gmtoff < 0 ? '-' : '+',
...
static u_char *
ngx_http_log_msec(ngx_http_request_t *r, u_char *buf, ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t)
{
    ngx_time_t  *tp;
	...
	return ngx_sprintf(buf, "%T.%03M", div_tp.sec, div_tp.msec);
    ...
    ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t)
{
    ...
    if(div_var_t->count != div_var_t->current_count)
    {
	ms=div_var_t->div_msec;
    }
    else
    {
        ms = div_var_t->remaining_msec;
    }
...
static u_char *
ngx_http_log_status(ngx_http_request_t *r, u_char *buf, ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t)
{
    ngx_uint_t  status;
...
static u_char *
ngx_http_log_substatus(ngx_http_request_t *r, u_char *buf, ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t)
{
    ngx_uint_t  status;
    ...
    return ngx_sprintf(buf, "%03ui.%d", status,div_var_t->current_count);
}
    ...
    ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t)
{
    ...
    if(div_var_t->count != div_var_t->current_count)
    {
	// ( length-(나머지 초의 용량) ) / (div_count -1)
	return ngx_sprintf(buf, "%O", div_var_t->sent_bytes_per_count_header);
    }

    return ngx_sprintf(buf, "%O", div_var_t->sent_bytes_remaining_header);
...
ngx_http_log_body_bytes_sent(ngx_http_request_t *r, u_char *buf,
    ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t)
{
//suyong9 여기서 나누어야 함: 초단위로 나눈뒤 시간별로 전송량 계산
    off_t  length;
   ...
   length = r->connection->sent - r->header_size;
	...
	{
		// ( length-(나머지 초의 용량) ) / (div_count -1)
		return ngx_sprintf(buf, "%O", div_var_t->sent_bytes_per_count);
	}
	else
	{
		// (나머지 초의 용량) 
		return ngx_sprintf(buf, "%O", div_var_t->sent_bytes_remaining);
	}
    }
...
static u_char *
ngx_http_log_variable(ngx_http_request_t *r, u_char *buf, ngx_http_log_op_t *op,ngx_http_log_div_var_t *div_var_t)
{
    ngx_http_variable_value_t  *value;

   // ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"suyong http test3-0 log : %d",op->data);
    value = ngx_http_get_indexed_variable(r, op->data);

    if (value == NULL || value->not_found) {
        *buf = '-';
//    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"suyong http test3-1 log : -");
        return buf + 1;
    }

    if (value->escape == 0) {
//    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"suyong http test3-2 log : %s",value->data);
        return ngx_cpymem(buf, value->data, value->len);

    } else {
//    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"suyong http test3-3 log : %s",value->data);
...
ngx_http_log_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if(conf->div == NGX_CONF_UNSET_PTR)
    {
	conf->div = prev->div;
        if (conf->div == NGX_CONF_UNSET_PTR) {
            conf->div = NULL;
...
static char *ngx_http_log_division_set(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) //#jimmy-4-3
{
    ...
    ngx_str_t *value;
    ngx_uint_t sectime=0;
   

    value=cf->args->elts;

    sectime=ngx_atoi(value[1].data,value[1].len); //error처리
    if(sectime<1 && sectime>3600) 
    {
            return NGX_CONF_ERROR;
    }


    llcf->div=ngx_palloc(cf->pool, sizeof(ngx_http_log_division_conf_t));
    if (llcf->div == NULL) {
            return NGX_CONF_ERROR;
    }

    llcf->div->sectime=sectime;
